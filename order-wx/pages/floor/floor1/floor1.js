...
Page({
  ...
  // },
  imgClick: function(e){
    var this_checked = e.currentTarget.id;
    var seatIdListCheck = this.data.seatIdList;
    for (let key in seatIdListCheck) {
      let i=0;
      let seatIdListCheckItem = seatIdListCheck[key];
      for (i = 0; i < seatIdListCheckItem;i++) {
        if (seatIdListCheckItem[i].seat_id==this_checked){
          seatIdListCheckItem[i].checked = true;
        } else {
          seatIdListCheckItem[i].checked = false;//其他的位置为false
        }
      }
    }
    console.log(seatIdListCheck);
    this.setData({
      seatIdList: seatIdListCheck
    })
    console.log(this.data.seatIdList.arrayOne);
  ...
  formSubmit: function(e){
    ...
    if(this.data.user.reputation<80){
      wx.showToast({
        title: "信誉分过低",
        icon: 'none',
        duration: 2000
